...
function D($name = '', $layer = '')
{
    ...
    if (class_exists($class)) {
        ...
        $model = new Common\Model\Model(basename($name));
    }
...
function U($url = '', $vars = '', $suffix = true, $domain = false)
{
    ...
    if (isset($host)) {
        ...
        if (C('APP_SUB_DOMAIN_DEPLOY')) {
            // 开启子域名部署  jry 7661660@qq.com 20160704
            ...
            foreach (C('APP_SUB_DOMAIN_RULES') as $key => $rule) {
                ...
                if (false === strpos($key, '*') && 0 === strpos($url, $rule)) {
                    ...
                    // 修复在URL小写时开启二级域名绑定模块出现控制器名未自动转换驼峰法为下划线
                    $urlExplode = explode(C('URL_PATHINFO_DEPR'), $url);
                    if (count($urlExplode) === 3) {
                        $urlExplode[1] = parse_name($urlExplode[1]);
                        $url           = implode(C('URL_PATHINFO_DEPR'), $urlExplode);
                    }
                ...
                // 子域名部署特殊转换 jry 7661660@qq.com 20160704
                // 将https://www.lingyun.net/forum转换成https://bbs.lingyun.net
                if (false === strpos($key, '*') && 0 === strpos($url, '/' . $rule)) {
                    $domain = $key . strstr($domain, '.'); // 生成对应子域名
                    $url    = substr_replace($url, '', 0, strlen('/' . $rule));
                    break;
    ...
    if (0 == C('URL_MODEL')) {
        ...
        if (isset($route)) {
            if (C('URL_ROUTER_ON')) {
                $_url = Think\Route::reverse($_module . $depr . lcfirst($url), $vars, $depr, $suffix);
            }
            if ($_url) {
                if (0 === strpos($_url, strtolower($_module) . '/')) {
                    $url = __APP__ . '/' . substr_replace($_url, '', 0, 1 + strlen($_module));
                } else {
                    $url = __APP__ . '/' . $_url;
                }
            } else {
            ...
            if (C('URL_ROUTER_ON')) {
                $_url = Think\Route::reverse($module . $depr . $path, $vars, $depr, $suffix);
            }

            if ($_url) {
                $url = __APP__ . '/' . $_url;
            } else {
                if ($module && strtolower(C('DEFAULT_MODULE')) === strtolower($module)) {
                    $url = __APP__ . '/' . $path;
                } else {
                    $url = __APP__ . '/' . ($module ? $module . MODULE_PATHINFO_DEPR : '') . $path;
                }
            }
        }
        if ($urlCase) {
