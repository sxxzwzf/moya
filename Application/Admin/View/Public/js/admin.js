$(function(){
    ...
    $(document).on('click', '.search-btn', function() {
        ...
        if(url.match(/&keyword=\d+/))
        {
            url.replace(/&keyword=\d+/,query);
        }
        else {
            if (url.indexOf('?') > 0) {
                url += '&' + query;
    ...
    $(function () {
        var perpage=10;
       if(window.location.href.match(/&perpage=\d*/g))
       {
           perpage=window.location.href.match(/&perpage=\d*/g)[0].split("=")[1]
       }

        $(".perpageselect").val(perpage);

        $(".perpageselect").bind("change",function(){
            var url=window.location.href;
            var pattern = /&perpage=\d*/;
            var dataname = $(this).val();
            if(window.location.href.match(pattern)){
                url=url.replace(pattern, "&perpage="+dataname)
            }
            else{
                if (url.indexOf('?') > 0) {
                    url = url+"&perpage="+dataname;
                } else {
                    url =url+ '?'  +"&perpage="+dataname;
                }
            }
