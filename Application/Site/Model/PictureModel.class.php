...
class PictureModel extends Model
{
     ...
     * @author jry <7661660@qq.com>
     */
    protected $tableName = 'site_picture';
    ...
    public function deletepictures($model=null,$ids,$status)
    {
            ...
            case 'delete': // 删除条目
                ...
                foreach ($id_list as $id) {
                    ...
                    if ($upload_info) {
                        ...
                        $psdid=$upload_info["psdid"];
                        $resut = D('Site/Picture')->delete($id);

                        //查找别的条目是否引用此图片，如果不引用则删除文件
                        ...
                        if($psdid)
                        {
                            $hasquot = D('Site/Picture')->where("psdid=".$psdid)->select();
                            if(!$hasquot)
                            {
                                $mupload=D("Admin/Upload");
                                $uploadfile=$mupload->find($psdid);
                                if(!preg_match("/home/",$uploadfile["path"]))
                                {
                                    //continue;
                                }
    ...
    protected $_validate = array(
        array('cid', 'require', '请选择图片分类', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH),
        array('picids', 'require', '请上传图片', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH),
