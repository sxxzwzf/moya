    ...
    public function addTopButton($type, $attribute = null)
    {
        ...
        switch ($type) {
            ...
            case 'delete': // 添加删除按钮(我没有反操作，删除了就没有了，就真的找不回来了)
                ...
                $this->_top_button_list[] = $my_attribute;
                break;
            case 'changecatagory': //添加自定义按钮(第一原则使用上面预设的按钮，如果有特殊需求不能满足则使用此自定义按钮方法)
                // 预定义按钮属性以简化使用
                // 预定义按钮属性以简化使用
                $my_attribute['title']       = '<i class="fa fa-stack-exchange"></i> 改分类';
                $my_attribute['target-form'] = 'ids';
                $my_attribute['class']       = 'btn btn-danger-outline btn-pill btn-secondary cat-modal';
                $my_attribute['model']       = $attribute['model'] ?: request()->controller();
                $my_attribute['data-toggle'] = "modal";
                $my_attribute['data-target'] = "#catModal";
                $my_attribute['data-index'] = "0";
                $my_attribute['href']        = U(
                    request()->module() . '/' . request()->controller() . '/setStatus',
                    array(
                        'status' => 'changecatagory',
                        'model'  => $my_attribute['model'],
                    )
                );
 ...
 */
     ...
     * @author jry <7661660@qq.com>
     */
    public function setsearch_form_items($name, $type, $title, $tip, $options = array(), $extra = '', $extra_attr = '',$url)
    {
        $item['name']           = $name;
        $item['type']           = $type;
        $item['title']          = $title;
    ...
    public function display()
    {
            ...
            foreach ($this->_table_column_list as &$column) {
                ...
                switch ($column['type']) {
                    ...
                    case 'picture':
                        ...
                        $column_value = '<a target="blank" href="'. get_cover($column_value) .'"><img class="picture" src="' . get_cover($column_value) . '"></a>';
                        break;
                    case 'pictures':
        ...
        $this->assign('search_form_items', $this->_search_form_items); // 搜索配置
